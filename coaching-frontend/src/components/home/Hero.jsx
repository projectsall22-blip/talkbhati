...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden pt-16">
      ...
      <div className="absolute top-1/4 left-0 w-64 h-64 rounded-full blur-3xl opacity-20 animate-float"
        style={{ background: 'radial-gradient(circle, #059669, transparent)' }} />
      <div className="absolute bottom-1/4 right-0 w-72 h-72 rounded-full blur-3xl opacity-15 animate-float"
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-24 w-full">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
              className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 mb-5 text-xs sm:text-sm font-medium text-emerald-300"
            ...
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
              className="text-3xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.15] mb-4">
              Your Pharmacy<br />
              Notes,{' '}
            ...
            <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              className="text-gray-300 text-sm sm:text-base leading-relaxed mb-6 max-w-lg">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
              ...
              <Link to="/notes"
                className="flex items-center justify-center gap-2 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition-all hover:scale-105 hover:opacity-90 shadow-lg"
              ...
              <Link to="/videos"
                className="flex items-center justify-center gap-2 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition-all hover:scale-105"
          ...
          {/* ── Right — founder card (desktop only) ── */}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
          className="mt-10 sm:mt-16 grid grid-cols-3 gap-3 max-w-lg">
          {[
            { icon: BookOpen,   value: '—', label: 'Study Notes' },
            { icon: Download,   value: '—', label: 'Downloads' },
            { icon: ArrowRight, value: '4', label: 'Programs' },
          ].map(({ icon: Icon, value, label }) => (
            <div key={label} className="flex items-center gap-2 sm:gap-3 rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3"
              style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
              <Icon className="w-4 h-4 text-emerald-400 shrink-0" />
              <div className="min-w-0">
                <div className="text-white font-bold text-xs sm:text-sm leading-none">{value}</div>
